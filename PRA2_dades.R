...
missile_attacks_daily <- missile_attacks_daily %>% 
        select(c(1:9)) %>% 
        mutate(time_start = as.POSIXct(time_start,format="%Y-%m-%d %H:%M"),
        ...
        separate(launch_place, into = c("launch_place1","launch_place2","launch_place3","launch_place4","launch_place5","launch_place6"), 
                 sep = " and ", fill = "right") %>%
